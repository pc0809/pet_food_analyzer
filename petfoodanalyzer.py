...
with preferences:
    ...
    filtered_data['Split_Individual_Ingredient'] = filtered_data['ingredient_panel'].str.split(', ')
    filtered_data['parent_category'] = filtered_data['parent_category'].str.split(', ')
    filtered_data['sub_category'] = filtered_data['sub_category'].str.split(', ')

    filtered_data_exploded = filtered_data.explode(['Split_Individual_Ingredient','sub_category' ,'parent_category'])
    ...
    # ingredient table column starts here
        with col_for_ingre_table:
            ...
            filtered_for_ing = filtered_data_exploded.groupby(['Split_Individual_Ingredient',
            'parent_category',
            ...
            filtered_for_ing = filtered_for_ing[filtered_for_ing['Products containing the ingredient']<filtered_data_exploded['Product'].nunique()-2]
    ...
    product_name_final = list(
        filtered_data_exploded[
            filtered_data_exploded['Split_Individual_Ingredient'].isin([each for each in deselected_ing_list])]['Product'].unique())
    ...
    final_filtered_data = filtered_data_exploded[~filtered_data_exploded['Product'].isin(product_name_final)]
    ...
    # final data to show
    final_data = final_filtered_data[['Product',
    'Price/Day ($)',
    'Fat (oz)',
    'fat_abs_diff_perc',
    'Protein (oz)',
    ...
    'protein_abs_diff_perc':'Deviation in Protein (%)'}).drop_duplicates().sort_values(by=['Price/Day ($)'])  
...
with best_products:
    ...
    with main_table_col:
        ...
        if len(list_of_final_products) != 0:
            ...
            final_filtered_data_1 = final_filtered_data[final_filtered_data['Product'].isin(list_of_final_products)][['Product', 'ingredient_panel']].drop_duplicates()
            
            ing_list = []
            for each in final_filtered_data_1['ingredient_panel'].str.split(', '):
                for each_2 in each:
                    ing_list.append((', '.join(each), each_2, standard_ingredient_dict[each_2]))
                    
            ing_df = pd.DataFrame(ing_list, columns = ['ingredient_panel', 'ingredient', 'category'])
